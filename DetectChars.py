...
def loadKNNDataAndTrainKNN():
    ...
    kNearest.train(npaFlattenedImages, cv2.ml.ROW_SAMPLE,
                   npaClassifications)           # train KNN object
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:
        ...
        listOfMatchingChars = findListOfMatchingChars(
            possibleChar, listOfPossibleChars)
        ...
        listOfPossibleCharsWithCurrentMatchesRemoved = list(
            set(listOfPossibleChars) - set(listOfMatchingChars))
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        ...
        fltDistanceBetweenChars = distanceBetweenChars(
...
def removeInnerOverlappingChars(listOfMatchingChars):
    listOfMatchingCharsWithInnerCharRemoved = list(
        listOfMatchingChars)                # this will be the return value
                ...
                if distanceBetweenChars(currentChar, otherChar) < (currentChar.fltDiagonalSize * MIN_DIAG_SIZE_MULTIPLE_AWAY):
                    ...
                    if currentChar.intBoundingRectArea < otherChar.intBoundingRectArea:
                        ...
                        if currentChar in listOfMatchingCharsWithInnerCharRemoved:
                            listOfMatchingCharsWithInnerCharRemoved.remove(
                    ...
                    # else if other char is smaller than current char
                        ...
                        if otherChar in listOfMatchingCharsWithInnerCharRemoved:
                            listOfMatchingCharsWithInnerCharRemoved.remove(
                                otherChar)           # then remove other char
                        # end if
                    # end if
                # end if
            # end if
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        cv2.rectangle(imgThreshColor, pt1, pt2, Display.SCALAR_GREEN,
        ...
        retval, npaResults, neigh_resp, dists = kNearest.findNearest(
            npaROIResized, k=1)              # finally we can call findNearest !!!
